...
int feuille(struct config conf, int *cout) // feuille means fin de partie
{
...
int estim(struct config conf)
{
	...
	// dans tout les cas on évalue le matériel, c'est à dire on compte le nombre de
	// pièces mais on va la modifier pour matcher les standards de jeux d'echec actuells

	int i, j, score = 0;
	int pionB = 0, pionN = 0, cB = 0, cN = 0, fB = 0, fN = 0, tB = 0, tN = 0, nB = 0, nN = 0;

	// material count (pieces worth)
	// calcule de la valeur cumulée des pièces on prenant en considération leurs puissances
	for (i = 0; i < 8; i++)
		for (j = 0; j < 8; j++)
		{
			switch (conf.mat[i][j])
	...
	// Somme des piéces : on prend les standards de jeu actuelles
	score += (pionB + cB * 3 + fB * 3 + tB * 5 + nB * 9);
	// printf("\nscore: %d %d %d %d %d %d\t", pionB, cB, tB, fB, nB, score);
	score -= (pionN + cN * 3 + fN * 3 + tN * 5 + nN * 9);
	// printf("\nscore: %d %d %d %d %d %d \t", pionN, cN, tN, fN, nN, score);
	//   On commance par la premiere phase de jeu (opening) nombre de coups < 20

	if (nb_coups < 20)
	{
		// le premier facteur dans l'opening sera king safety (sécurité du roi)
		// pour evaluer ceci on verifie premierement une ou les deux roques sont réalisables
		int safetyBK = 0, safetyWK = 0;
		// roi blanc
		switch (conf.roqueB)
		{
		case 'e':
			safetyWK += 10; // king safer
			break;
		case 'r':
			safetyWK += 5;
			break;
		...
		// meme chose pour le roi noir
		switch (conf.roqueN)
		{
		case 'e':
			safetyBK += 10; // king safer
			break;
		case 'r':
			safetyBK += 5;
			break;
		...
		// puis on evalue l'espace devant le roi blanc et on voie si il ya des
		// des pièces blanches ou noires au tour de lui
		// si ils sont blanc on augmante white king safety sinon on la diminue
		int stop, depx, depy;
		for (i = 0; i < 8; i++)
		{
			stop = 0;
			depx = conf.xrB + D[i][0];
			depy = conf.yrB + D[i][1];
			while (!stop && depx >= 0 && depx <= 7 && depy >= 0 && depy <= 7)
				if (conf.mat[depx][depy] != 0)
					stop = 1;
				else
				{
					depx = depx + D[i][0];
		...
		// meme chose pour le roi noir juste l'inverse des couleurs
		for (i = 0; i < 8; i++)
		{
			stop = 0;
			depx = conf.xrN + D[i][0];
		...
		// on ajoute le critere de safety (sécurité) a l'estimation
		score += safetyWK;

		score -= safetyBK;

		// maintenant on voi le développement des pièces (qui a bien developpé ces pièces
		// dans la phase opening ) ainsi que si le roi est encore dans le centre ou non

		// premièrement on voi combien de pions du centres et de pièces légères on été développées
		// meaning they left the home square
		// the light pieces are nights and bishops (fous et cavaliers)
		int devB = 0, devN = 0;
		...
		// deuxiemement on évalue l'occupation du centre par les différents pièces
		int bonus, blackCenter = 0, whiteCenter = 0;
		for (i = 0; i < 8; i++)
		{
			for (j = 0; j < 8; j++)
			{
		...
		whiteCenter = whiteCenter * 0.1; // pour ne pas abuser l'estimation
		blackCenter = blackCenter * 0.1;
		// I might change this factor

		score += whiteCenter;
		score -= blackCenter;
	}

	// Deuxième phase de jeu (middle game) nombre de coups entre 20 et 35
	// pour cette partie on consédère le développement des pièces ainsi que
	// l'occupation du centre
	if (nb_coups >= 20 && nb_coups <= 35)
	{

		// puis on regarde le developpement des pièces ainsi que l'occupation du centre
		// premièrement on voi combien de pions du centres et de pièces légères on été développées
		// meaning they left the home square
		// the light pieces are nights and bishops (fous et cavaliers)
		int devB = 0, devN = 0;
	...
	// pour la dèrnière phase de jeu (end game) on regarde king safety (sans roques)
	// ainsi que le nombre de cases menacées par chaque pièce de chaque joueur
	// on utilise pour cela la fonction déja implémentée caseMencaeePar
	...
	if (nb_coups > 35)
	{
		...
		// cases occupées
		int whiteTeratory = 0, blackTeratory = 0;
		for (i = 0; i < 8; i++)
			for (j = 0; j < 8; j++)
			{
				if (conf.mat[i][j] < 0 && caseMenaceePar(MAX, i, j, conf))
				{ // case courante menacée par une des pièces blanches
					whiteTeratory++;
				...
				{ // case courante menacée par une des pièces noires
					whiteTeratory++;
					if (conf.mat[i][j] == 'c' || conf.mat[i][j] == 'f')
						blackTeratory += 2;
					if (conf.mat[i][j] == 't' || conf.mat[i][j] == 'n')
		...
		score += (whiteTeratory - blackTeratory) * 0.3;
		// might change the factor
	}
...
int main(int argc, char *argv[])
{
	...
	printf("\n\nVous etes les + (Blancs) et je suis les - (Noirs)\n\n");
	...
	while (!stop)
	{
		...
		{ // Estimation à la profondeur spécifiée dans sx
